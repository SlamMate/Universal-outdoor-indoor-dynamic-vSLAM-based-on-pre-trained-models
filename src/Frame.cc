     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    if(imGrayPre.data)
    {
        ...
        ProcessMovingObject(imLeft);//传入的是灰度图
...
void Frame::SetFig_area(double newFig_area)
{
	fig_area = newFig_area;
}
...
void Frame::RemoveOutliers(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera, Frame* pPrevF, const IMU::Calib &ImuCalib){
	...
	std::vector<cv::Rect> boxes_dynamic_T;
	...
	if(boxes_dynamic.size() > 0){
		...
		// 对潜在动态框进行判断，1.哪个潜在动态框有离群点，如果没有离群点则认为是静态的，从数组中剔除。2.潜在变换框分为四个均匀的小矩形，哪个部分有离群点，确定了哪个部分之后，将height进行除2,将框缩小为四分之一，可以改进为如果有多个离群点落在一个区域才为动态区域，增加鲁捧性
		int n_dynamic = 0;
		while(n_dynamic < boxes_dynamic.size()){
				...
				//储存每个框的关键点信息
				std::vector<cv::KeyPoint> _mvKeys_1;
				cv::Mat _mDescriptors_1;	
				
				for (int i=0; i < T_M.size(); ++i){
				T = T_M[i];
				if(T.y > box1.y && T.y < box1.y + box1.height && T.x > box1.x && T.x < box1.x + box1.width){
					n_T++;
				}
								}
				...
				for (size_t i(0); i < nextpoint.size(); ++i){
					//将框内点和数目记录下来		
					if(nextpoint[i].y > box1.y && nextpoint[i].y < box1.y + box1.height && nextpoint[i].x > box1.x  && nextpoint[i].x < box1.x + box1.width){
						n++;
						}
				}
				//std::cout << (float)n_T/(float)n << std::endl;
				//动态则放入动态框中，非动态则删去,防止噪声点，两个以上的离群点才能判定动态
				// 为了防止只有是少数的点被遮挡，物体本身处于静态的情况，因为被遮挡时，很多都是两个两个离群点
				// 仍然无法解决相机相对运动被认为是本身运动的问题，这个或许必须从全局来解决。
				if( (float)n_T/(float)n>0.4) {
					is_dynamic=true;
					//std::cout << "zhong" << (float)n_T/(float)n << std::endl;
				}
				//如果是动态的就将框内点删去
				//如果是静态的话就加入关键点
				if(is_dynamic){
					boxes_dynamic_T.push_back(box1);
				}else{
					boxes_stay.push_back(box1);
				}
				n_dynamic++;
			}
	}
	//这部分是对人的框的处理，人是非刚体，可以局部进行运动，所以设置为一个经验上较小的阈值1，而不是根据总提取的点数.
	if(boxes_half.size() > 0) {
		int n_half = 0;
		while(n_half < boxes_half.size()){
				box1 = boxes_half[n_half];
		...
		for (size_t i(0); i < mvKeys.size(); ++i){
			...
			while(n_dynamic < boxes_dynamic_T.size()){
				box1 = boxes_dynamic_T[n_dynamic];			
				if(mvKeys[i].pt.y > box1.y && mvKeys[i].pt.y < box1.y + box1.height && mvKeys[i].pt.x > box1.x  && mvKeys[i].pt.x < box1.x + box1.width){
					bool_key = true;
					// std::cout << bool_key << std::endl;
				}
				n_dynamic++;

			}
			// std::cout << boxes_stay.size() << std::endl;
			...
			//否则为false,将关键点放在图中
			//在框内就是true，为异常点就是true
			//只剔除在框内且为异常点的
			//A交B的反是!A并!B
			//这个点即在框内又是离群点的话，则剔除整个框内的点？
			if(!bool_key)
			{      			
...
//单目的改动
void Frame::RemoveOutliers(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF, const IMU::Calib &ImuCalib)
{
...
//上面的frame对应的动态点去除
void Frame::RemoveOutliers(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera, GeometricCamera* pCamera2, Sophus::SE3f& Tlr,Frame* pPrevF, const IMU::Calib &ImuCalib){
...
/**			while(n_peopleRight < boxes.size()){
		...
		mvKeys = _mvKeys;
    		mDescriptors = _mDescriptors;
    		mvKeysRight = _mvKeys;
    		mDescriptorsRight = _mDescriptors;
	}
