...
void Detector::Run()
{
	...
	//一微秒运行一次
		...
		// 检测图像的通道数
		// std::cout << mImg.channels() << std::endl;
		//std::cout << mImg.cols << std::endl;
		//std::cout << mImg.rows << std::endl;
...
float* blobFromImage(cv::Mat& img){
    ...
    for (size_t c = 0; c < channels; c++) 
    {
        for (size_t  h = 0; h < img_h; h++) 
        {
            ...
            {
                //std::cout << (float)img.at<cv::Vec3b>(h, w)[c] << std::endl;
...
//    int stay_counter {0};
    ...
    for (size_t i = 0; i < objects.size(); ++i){
		...
		box1 = objects[i].rect;
		//如果框的宽度大于长度，就认为是坐下了
		if(objects[i].rect.height / objects[i].rect.width <=3){
		//上半身放入盒子中，不做处理
		//y越小，上半身越大
		objects[i].rect.y = objects[i].rect.y+objects[i].rect.height/1.3;
		// std::cout << objects[i].rect.height << std::endl;
		objects[i].rect.height = objects[i].rect.height*0.3/1.3;
		
		half_counter++;
		half_boxes.push_back(objects[i].rect);
		// std::cout << objects[i].rect.height << std::endl;
			
		//下半身放入对极几何盒子中，进行检查
		//height越大，下半身越大
		box1.height = box1.height/1.3;
		half_counter++;		
		half_boxes.push_back(box1);
		}else{
		half_counter++;		
		half_boxes.push_back(box1);
		}
	}else{
		//对于其他物体上下框分开判断，是因为噪声点抑制算法过于严格，有时候对于细小的动作检测不出来,也可以对一些局部运动的物体更有高性能
		//如只有一半在转的转椅。
		/**cv::Rect box1;
		box1 = objects[i].rect;
		//高大于宽，就上下分成两部分
		if(objects[i].rect.height >= objects[i].rect.width){
		objects[i].rect.y = objects[i].rect.y+objects[i].rect.height/2;
		objects[i].rect.height = objects[i].rect.height/2;	
		dynamic_counter++;		
		dynamic_boxes.push_back(objects[i].rect);
		
		box1.height = box1.height/2;
		dynamic_counter++;		
		dynamic_boxes.push_back(box1);
		}
		//宽大于高，就分为左右两个部分，比如键盘这类物体，高度很小，分上下意义不大
		else{
		objects[i].rect.x = objects[i].rect.x+objects[i].rect.width/2;
		objects[i].rect.width = objects[i].rect.width/2;	
		dynamic_counter++;		
		dynamic_boxes.push_back(objects[i].rect);
