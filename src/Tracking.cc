...
//	mCurrentFrame.SetBoxes_stay(mpDetector->stay_boxes);
 ...
 if (mSensor == System::STEREO && !mpCamera2)
        ...
        mCurrentFrame.RemoveOutliers(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr);
    else if(mSensor == System::IMU_STEREO && !mpCamera2)
        mCurrentFrame.RemoveOutliers(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    else if(mSensor == System::IMU_STEREO && mpCamera2)
        mCurrentFrame.RemoveOutliers(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr,&mLastFrame,*mpImuCalib);
...
// FEATURE NUMBER UPDATE
	...
	// nFeatures = FeatureNumberUpdate(mCurrentFrame.aprox_area, mpDetector->fig_factor);
	//std::cout << "Feature number: " << nFeatures << std::endl;
...
//	int keypoints = 1000*factor;
	//那个factor的意义没搞懂，所以就按1来了
	int keypoints = 1000*1;
...
//	mCurrentFrame.SetBoxes(mpDetector->people_boxes);
	mCurrentFrame.SetBoxes_half(mpDetector->half_boxes);
	mCurrentFrame.SetBoxes_dynamic(mpDetector->dynamic_boxes);
//	mCurrentFrame.SetFig_area(mpDetector->fig_area);
///	mCurrentFrame.SetBoxes_stay(mpDetector->stay_boxes);
	// remove dynamic features
	 if (mSensor == System::MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame.RemoveOutliers(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
        else
            mCurrentFrame.RemoveOutliers(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
    }
    else if(mSensor == System::IMU_MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        {
            mCurrentFrame.RemoveOutliers(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
        }
        else
            mCurrentFrame.RemoveOutliers(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
    }
