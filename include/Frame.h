...
//    std::vector<cv::Rect> detectedBoxes;
    std::vector<cv::Rect> detectedBoxes_dynamic;
    std::vector<cv::Rect> detectedBoxes_half;
//    std::vector<cv::Rect> detectedBoxes_stay;
//    void SetBoxes(std::vector<cv::Rect> newBoxes);
    void SetBoxes_dynamic(std::vector<cv::Rect> newBoxes);
    void SetFig_area(double newFig_area);
    void SetBoxes_half(std::vector<cv::Rect> newBoxes);
//    void SetBoxes_stay(std::vector<cv::Rect> newBoxes);

	// RGB-D
    void RemoveOutliers(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
	// Monocular
    void RemoveOutliers(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
	// Stereo
